...
export function directives(el, attributes, originalAttributeOverride) {
  ...
  if (el._x_virtualDirectives) {
    ...
    // Handle binding normal HTML attributes (non-Stator directives).
...
export function attributesOnly(attributes) {
  return Array.from(attributes)
    ...
    .filter(attr => !outNonStatorAttributes(attr));
}
...
function outNonStatorAttributes({ name }) {
...
let directiveOrder = ['ignore', 'ref', 'data', 'id', 'anchor', 'bind', 'init', 'for', 'model', 'modelable', 'transition', 'show', 'if', DEFAULT, 'teleport'];
